...
public class QuestionBankServiceImpl extends ServiceImpl<QuestionBankMapper, QuestionBank> implements QuestionBankService {
    ...
    @Override
    public PageResponse<BankHaveQuestionSum> getBankHaveQuestionSumByType(String username, String bankName, Integer pageNo, Integer pageSize) {
        if (username.equals("ll")) { // 最高超级管理员
...
//              这个不是ll 用户  普通用户 并不能进行查询所有题库信息
        } else {
            List<WangkeyewuVo> allYeWu = noticeService.getAllYeWu(username); //用户名称
            // 显示一个字段数据 用户拥有的业务
            List<String> collectyewuQuBinkname = allYeWu.stream().map(WangkeyewuVo::getYewuQuBinkname).collect(Collectors.toList());
            //String[]转String
            System.out.println(collectyewuQuBinkname);
            String id = String.join(",", collectyewuQuBinkname);
            //String转String[]
            String[] ids = id.split(",");
            QueryWrapper<QuestionBank> wrapper = new QueryWrapper<>();
            wrapper.in("bank_name", ids);  //数组查询
            IPage<QuestionBank> iPage = questionBankMapper.selectPage(new Page<>(pageNo, pageSize), wrapper);
            List<QuestionBank> questionBanks = iPage.getRecords();
            List<BankHaveQuestionSum> bankHaveQuestionSums = new ArrayList<>();
            for (QuestionBank questionBank : questionBanks) {
