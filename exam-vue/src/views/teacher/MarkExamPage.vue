    ...
    <el-header height="180">
      <el-card style="height: 180px">
        ...
        <el-row style="margin-top: 55px;">
          <el-tooltip class="item" effect="dark" content="包括(单选,判断题)" placement="top-start">
...
<!-- 
    ...
    <el-main>
      <el-card>
        ...
        <div v-for="(item,index) in questionInfo" :key="index" style="margin-top: 15px">
          ...
          <!--题目中的配图-->
          <img v-for="url in item.images" :src="url" :key="url" title="点击查看大图" alt="题目图片"
              ...
              :key="index2"
                ...
                <img style="position: absolute;left:100%;top:50%;transform: translateY(-50%);
                  width: 40px;height: 40px;float: right;cursor: pointer;" title="点击查看大图"
                     v-for="i3 in i2.images"  :key="i3" :src="i3" alt="" @mouseover="showBigImg(i3)">
              </label>
            </div>
          </div>
    ...
    <el-dialog :visible.sync="creditDialog" @close="creditDialog = false">
      <img style="width: 100%" :src="item"   :key="item" v-for="item in examRecord.creditImgUrl.split(',')">
    </el-dialog>
  </el-container>
</template>
